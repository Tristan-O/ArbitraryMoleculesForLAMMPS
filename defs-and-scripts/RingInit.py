...
def AtomArange(Lx, Ly, Lz, n_p, N, ns, Rc):

    """ Arranges the polymer atoms, N, into a ring.
        Output:
            Pos(N,3) - position vector
    """
    numAtoms = n_p*N + ns + 2*bool(Rc) # the first 2 comes from the ends of the polymer
    ...
    rx = np.linspace(-0.5*LatScale*Lx,0.5*LatScale*Lx, n_p, dtype=float)
    ...
    # Not sure if we want polymers entrapping each other
    for x in rx:
        for t in theta:
            x = 
...
if __name__ == "__main__":
    ...
    parser.add_argument('-N','--DOP',type=int,default=10, help='specify the degree of polymerization, number of statistical segments')
    ...
    AtomArange(args.LboxSidex,args.LboxSidey,args.LboxSidez, args.numberpolymer, args.DOP, args.numbersolvent, args.colloidRadius)