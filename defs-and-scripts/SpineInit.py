...
def AtomArange(Lx, Ly, Lz, n_p, N1, N2, ns, Rc):

    """ Arranges the polymer atoms, N1 and N2 and 2 end-beads, onto a lattice.
            Polymer looks like: 1-2-2-2-...-3-3-3-1
            Output:
                Pos(N,3) - position vector
"""
    # N is the total number atoms in the system (DOP*P + ns)
    N = n_p*(2 + N1 + N2) + ns + 2*bool(Rc) # the first 2 comes from the ends of the polymer
    ...
    data(Pos, Lx,Ly,Lz, n_p,N1,N2, ns, Rc)
    
    

def data(Pos, Lx,Ly,Lz, n_p,N1,N2, ns, Rc):
...
"""
    ...
    while i <= NumAtoms: 
        ...
        if i <= numpolyatoms:
            if k == NumAtomsP or k == 1:
                ...
                if numN1 <= N1:
                    f.write("2 ") # the atom is in the middle of the polymer, in the first block
                    numN1 += 1
                elif numN2 <= N2: # the atom is in the middle of the polymer, in the second block
                    f.write("3 ")
                    numN2 += 1
                else:
                    print 'Error: Mistake in algorithm has tried to assign an atom in a polymer '
            ...
            f.write("4 ") # the molecule is a solvent molecule
            ...
            f.write("5 ") # the molecule is a colloid
    ...
    while k < numpolyatoms:
        ...
        f.write("%i " % i)
        if j == 1:
            f.write("1 ") # the bond is end-to-N1
        elif j <= N1:
            f.write("2 ") # the bond is N1-to-N1
        elif j == N1 + 1:
            f.write("3 ") # the bond is N1-to-N2
        elif j > N1+1 and j<=N1+N2:
            f.write("4 ") # the bond is N2-to-N2
        elif  j == NumAtomsP - 1:
            f.write("5 ") # the bond is N2-to-end
        else:
            print "Mistake in your algorithm: bonds"
            sys.exit()
        f.write("%i %i\n" % (k, k+1)) 
        j = j + 1
        i = i + 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('-N1','--num1',type=int,default=10, help='specify the number of polymer beads of type 2, number of statistical segments')
    parser.add_argument('-N2','--num2',type=int,default=10, help='specify the number of polymer beads of type 2, number of statistical segments')
    ...
    AtomArange(args.LboxSidex,args.LboxSidey,args.LboxSidez, args.numberpolymer, args.num1, args.num2, args.numbersolvent, args.colloidRadius)