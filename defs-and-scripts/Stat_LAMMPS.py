...
class DEBUG:
  '''simple debug class that prints and writes to a string to save output...
  This should replace the built-in print function'''
  def __init__(self):
    self.out = ''
    return
  ...
  def get(self):
    return self.out
dbg = DEBUG() # Global definition of debugger
...
def doStats(warmupdata,Data,doGraphs=False,doWriteStdout=False,graphFilenameStub=''):
    ...
    if doWriteStdout == True:
        dbg.debug( "  - Mean                    = ",mean," +/- ",semcc)
        dbg.debug( "  - Equilibrated samples    = ",nsamples)
        dbg.debug( "  - Correlation time        = ",kappa)
        dbg.debug( "  - Effective # samples     = ",nsamples/kappa)
        dbg.debug( "  - Reduced-bias variance   = ",unbiasedvar)
        ...
        dbg.debug( "  - S.D. (unbiased, biased) = ",np.sqrt(unbiasedvar),np.std(Data,ddof=0)) # ddof is correction to 1/N...using ddof=1 returns regular reduced-bias estimator
        dbg.debug( "  - Skewness                = ",skew)
        dbg.debug( "  - Kurtosis                = ",kurtosis)
        dbg.debug( "  - Min, Max                = ",min,max)
        dbg.debug()  # Reduced bias estimator - test vs. above from sqrt(var)
...
def main(filestr, warmup=100, graph=False, quiet=False,autowarmup=False, col=None, observable=None):
  # For command-line runs, build the relevant parser

  #Testing above defined DEBUG class
  # dbg = DEBUG()
  # dbg.debug('Hello', 1, 2)
  # print dbg.get()
  ...
  if args.col != None:
    for i in args.col:
      if not args.quiet:
        dbg.print( "Processing column index {0}".format(i))
      if args.autowarmup:
          warmup,Data,nwarmup = autoWarmupMSER(args.file, i)
          if not args.quiet:
            dbg.debug( "Auto warmup detection with MSER-5 => ",nwarmup)
      else:
    ...
    for i in args.observable:
      if not args.quiet:
        dbg.debug( "Processing observable {0}".format(i))
